...
MODULE_AUTHOR("IV-83 Matviychuk Andrey");
MODULE_DESCRIPTION("Programm Hellow World Lab7");
MODULE_LICENSE("Dual BSD/GPL");

static uint amount = 1;
module_param(amount, uint, 0444);
MODULE_PARM_DESC(amount, "Hello Worlds count");
...
static int __init hello_init(void)
{
	...
	pr_info("Type the amount: %d\n", amount);

	if (amount == 0 || (amount >= 5 && amount <= 10))
		pr_warn("WARNING\nThe parameter is range from 0 to 10");
	BUG_ON(amount > 10);
